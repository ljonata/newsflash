...
- **House interior** (`buildInterior()`): the Home is enterable. Pressing E on Home runs `enterHouse()`, which opens a hinged front door (`makeDoor()`/`setDoorOpen()`, animated in `updateDoors()`) then, after a ~500ms `transitioning` swing, teleports the player to a furnished room far away at (`INT_CX`, `INT_CZ`) = (300, 300) — intentionally roomier than the exterior ("bigger on the inside"). Fog (`far = 180`) hides the room from the city and vice-versa, so no scene-swap is needed. The room has wood floor, ceiling, four solid walls, a cozy `PointLight`, back-wall windows, a carpet, a table + chair, and interactables 🛏️ Bed (sleep), 🧊 Fridge (opens the food menu), 🛋️ Sofa (relax), plus a hinged 🚪 exit door. `insideHouse` switches `updateNearBuilding()` to the `interior` list, `inBounds()` to the room box instead of `CITY_LIMIT`, and suppresses `emitState()` so remote players see you parked at the door
