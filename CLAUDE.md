...
| 07 | **Ninja Dash** | `/games/07/` | 2D side-scrolling platformer on canvas. A pixel-art ninja runs/jumps through a level breaking floating mystery boxes for hidden treasures while dodging spike & saw traps, then reaches the ⛩️ gate. Easy/Medium/Hard difficulty; iPad touch controls with JUMP on the left, movement on the right. |
...
- Interactive buildings: 🏠 Home, 🏢 Office, 🍔 Diner, 🌳 Park (four corners) plus 🛒 Market at block (2,0) for buying food. Proximity uses `distToFootprint()` (distance to the building's edge, threshold ~3.4) — NOT distance to center, so you trigger the prompt standing at any wall/corner. The Home exterior uses a procedural brick texture (`makeBrickTexture()`, a `CanvasTexture` with `RepeatWrapping`) on its wall faces plus a solid roof material (per-face materials array; BoxGeometry top face is index 2). Other blocks are decorative towers with lit-window planes
- **House interior** (`buildInterior()`): the Home is enterable. Pressing E on Home runs `enterHouse()`, which opens a hinged front door (`makeDoor()`/`setDoorOpen()`, animated in `updateDoors()`) then, after a ~500ms `transitioning` swing, teleports the player to a furnished room far away at (`INT_CX`, `INT_CZ`) = (300, 300) — intentionally roomier than the exterior ("bigger on the inside"). Fog (`far = 180`) hides the room from the city and vice-versa, so no scene-swap is needed. The room has wood floor, ceiling, four walls textured with a procedural "cool" wallpaper (`makeWallpaperTexture()` — cool-toned stripes + diamond motifs, applied via `wallMesh()`), a cozy `PointLight`, back-wall windows, a carpet, a table + chair, and interactables 🛏️ Bed (sleep), 🧊 Fridge (opens the food menu), 🛋️ Sofa (relax), plus a hinged 🚪 exit door. `insideHouse` switches `updateNearBuilding()` to the `interior` list, `inBounds()` to the room box instead of `CITY_LIMIT`, and suppresses `emitState()` so remote players see you parked at the door
- **Food system**: `foodTypes` (apple/sandwich/salad/pizza, each with hunger/mood/price) and `fridgeStock` (persisted in `citylife_state`). The Fridge opens a panel (`openFoodPanel('fridge')`) to **eat** items — stock decrements and can run out; the Market opens the same panel in **buy** mode to restock for cash. `menuOpen` pauses movement while the panel is up
...
- Life-sim stats: `cash`, `energy`, `hunger`, `mood`. Stats decay over time in the loop; hitting 0 hunger accelerates energy/mood loss; mood drifts toward neutral. `doAction()` keys off `nearBuilding.type` and advances in-game time: Office work earns cash, Diner eat costs $15, Park relax boosts mood, Home enters the house, Market opens the buy panel; indoors the Bed sleeps (energy → 100, advance to next 07:00), the Fridge opens the eat panel, and the Sofa relaxes
...
- **Level generation** (`buildLevel()`, uses `Math.random` — single-player, no sync needed): ground band split into segments with occasional pits (falling in costs a heart via `respawn()`); floating wood platforms kept within a double-jump of the ground; one **mystery box** per item floated in a band that is always within a *single* jump of the ground beneath it (derived from `jumpApex`, so every box is guaranteed reachable); **spikes** on ground segments; **saw blades** patrolling wider platforms; an **exit ⛩️ gate** on the final ground segment
...
- **Split touch controls**: the JUMP button sits bottom-**left** (`#controls-left`, for the left hand) and the move D-pad (◀ ▶) bottom-**right** (`#controls`), both shown only on touch devices (`ontouchstart`/`maxTouchPoints`); `bindHold()` wires touch + mouse press/release. Keyboard: ← → / A D to move, Space / ↑ / W to jump
